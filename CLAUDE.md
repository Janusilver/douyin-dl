...
```
3. **GUI / exe**：`gui.py` 自动分流四个平台，Cookie 文件名固定为 exe 同目录的 `douyin_cookies.txt` / `xhs_cookies.txt` / `kuaishou_cookies.txt`。
...
- `kuaishou.py`：链接→photoId（短链跳转后 URL/query 里取）→ curl_cffi GET `short-video/{id}` → 解析 `window.__APOLLO_STATE__`（JSON 尾部有 IIFE，需剥掉 `;(function(){var s;...}());`）的 `defaultClient["VisionVideoDetailPhoto:{id}"]`。**视频**：`manifest.adaptationSet[].representation[].url`（无水印，取最后最高清），兜底 `photoUrl`；**图集**：`ext_params.atlas`（JSON 字符串/列表）里的 cdnUrls。作者昵称从 `VisionVideoDetailAuthor:{uid}` 取。CDN 下载要带 `Referer: https://www.kuaishou.com/`。
...
- `kuaishou.py` 已实测通过（2026-08-15）：真实作品页链接（`short-video/3x7edaa985qmhqy`），匿名与带 Cookie 均成功，manifest 原档 mp4 落盘。
- `xhs.py` 已实测通过（2026-08-15，带登录 Cookie）：图文笔记 3/3 原图（走 `fileId` → `sns-img-bd.xhscdn.com`）、视频笔记 mp4（`media.stream.EF4` 最高清）。实况图（Live Photo）代码已支持（`imageList[].stream.EF4` masterUrl），未翻到样本实测。裸 `explore/{id}` 无 xsec 会被风控，需分享链接（带 xsec_token）或恰好出现在首页 feed。
